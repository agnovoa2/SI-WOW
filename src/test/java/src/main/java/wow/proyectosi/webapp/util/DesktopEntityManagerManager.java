 ...
 *
 * A ZK Desktop is created per complete page. That is, when user refreshes the
 * page or opens a new tab, or a new browser session, a new Desktop is created
 * for this new page.
 *
 * Mapping an EntityManager to Desktops is useful to keep an opened
 * EntityManager while the user is interacting with the same page, which
 * facilitates keeping entity objects in session (e.g. a ViewModel) while
 * keeping their persistent state, and thus avoiding, for example, lazy
 * initialization exceptions, the need to make merges, re-finding entities by
 * id, etc.
 */
